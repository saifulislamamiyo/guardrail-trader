...
- ACTIVE MODE (since 22 Sep 2026): **Docker**, run from this main folder (`docker compose up -d`; branch `docker` merged into main). launchd is in `docker-mode` (only keep-awake). Native IB Gateway app is NOT used. Rollback: `docker compose down` → open/log in native IB Gateway → `scripts/launchd.sh install`. Pre-switch journal backup: `data/journal.pre-docker-*.sqlite`.
