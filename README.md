...
`submit_orders`, so it can see why an order was blocked, and again, authoritatively, in `run_bot.py`
...
|---|---|---|
| Only allowlisted instruments (50 tickers) | `config/universe/*.csv` | `risk/gate.py` → `_check()`; list loaded by `risk/config.py` → `load_risk_config()` |
| Max single holding | 25% of portfolio | `risk/gate.py` → `_check()` (BUY branch) · `config/risk.toml` `max_position_pct` |
| Max orders per calendar month (Sydney) | 10 | `risk/gate.py` → `_check()` · count from `journal.py` → `orders_this_month()` |
| Kill switch: drawdown from peak | 25% → sell all, halt | `risk/gate.py` → `kill_switch_triggered()`, `liquidation_proposals()` · `run_bot.py` step 3 |
| Halt persists until manually reset | — | `journal.py` → `halt()` / `reset_halt()` · `scripts/journal_cli.py reset-halt --confirm` |
| No shorting: sell only what is held | hard-coded | `risk/gate.py` → `_check()` (SELL branch) |
| No margin: cost plus commission ≤ cash | hard-coded | `risk/gate.py` → `_check()` (BUY branch) |
| Limit orders only, within ±2% of reference price | `max_price_deviation_pct` | `risk/gate.py` → `_check()` |
| Commission ≤ 10% of order value | `max_commission_pct` | `risk/gate.py` → `_check()` · estimate from IBKR what-if `broker/ibkr.py` → `preview_order()` |
| Whole shares only (IBKR API rejects fractional orders) | `allow_fractional = false` | `risk/gate.py` → `_check()` |
| Invalid config refuses to load (fail-closed) | — | `risk/config.py` → `load_risk_config()`, `_pct()` |
...
|---|---|---|
| **Tools** | Claude gets read-only tools (`get_portfolio`, `list_universe`, `get_price_history`, `check_orders`) plus one final action (`submit_orders`). It never talks to the broker. | `agent.py` → `TOOLS`, `_dispatch()` |
| **Permissions** | The gate decides; the executor only places orders the gate approved. | `run_bot.py` step 5 · `executor.py` → `execute()` |
| **Paper vs live safety** | Paper mode refuses IBKR's live ports; the account ID must match the mode (paper IDs start with `DU`); read-only connections can't place orders. | `broker/ibkr.py` → `check_endpoint()`, `check_account()`, `place_order()` |
| **Reconciliation** | Before each run, the journal's holdings must equal IBKR's positions, with no stray open orders; otherwise it stops. | `run_bot.py` step 1 |
| **Virtual budget** | The paper account holds far more than the budget; the bot only sees its own ledger's cash. | `journal.py` → `init_budget()`, `cash_base()`, `portfolio_state()` |
...
The loop is in `agent.py` → `TradingAgent.run()`. It stops when:
...
|---|---|---|
| Virtual trading capital | A$5,000 during the paper phase | `config/risk.toml` `[budget]` · `journal.py` → `init_budget()` |
| LLM cost per run | stop at **US$0.10**, no trades | `.env` `LLM_RUN_BUDGET_USD` · `agent.py` → `TradingAgent.run()` |
| LLM cost per month | skip runs once **US$2.00** is spent | `.env` `LLM_MONTHLY_BUDGET_USD` · `run_bot.py` step 4 · `journal.py` → `llm_spend_usd()` |
| Cost measured from real usage | tokens × published prices (incl. cache and web search) | `llm.py` → `PRICES`, `cost_usd()` · table `llm_usage` |
| Unpriced models refused | a model with no known price can't be used | `llm.py` → `model()` |
| Prompt caching | system prompt, tools and conversation prefix cached | `agent.py` → `run()`, `_move_cache_breakpoint()` |
| No LLM call when the market is closed | weekends, holidays, outside slots | `run_bot.py` (before step 4) · `scripts/scheduled_run.py` |
| Small universe | 50 tickers keeps prompts small and pricing fast | `config/risk.toml` `[universe]` · `scripts/refresh_universe.py` |
| Web search off by default | each search costs extra | `.env` `ENABLE_WEB_SEARCH` · `agent.py` → `WEB_SEARCH_TOOL` |
| Provider choice | Anthropic direct or OpenRouter (same Messages API) | `llm.py` → `make_client()` |
...
| `config/risk.toml` | Budget, risk limits, universe files |
| `config/universe/saif_picks.csv` | Owner-picked tickers |
| `config/universe/top_sp500.csv` | Largest S&P 500 names by SPY weight, which top the universe up to 50; refresh monthly with `scripts/refresh_universe.py` |
...
```
guardrail_trader/
  broker/ibkr.py      IBKR adapter: executes, never decides; paper/live safety checks
  risk/gate.py        the risk gate (pure functions)
  risk/config.py      loads + validates risk.toml and the universe
  agent.py            Claude agent loop, tools, caching, per-run cost stop
  llm.py              provider switch, prices, cost accounting, caps
  journal.py          SQLite: ledger, runs, proposals, orders, snapshots, LLM usage
  executor.py         places approved orders, books fills
  market.py           prices, history summaries, market hours
  dashboard_data.py   read-only views for the dashboard
  web/dashboard.html  dashboard UI
scripts/              run_bot, scheduled_run, launchd.sh, dashboard, journal_cli, check_connection, …
tests/                gate, broker safety, journal, agent, LLM budget, dashboard
```
...
(`guardrail_trader/fake_llm.py`) exercises the full tool protocol.
